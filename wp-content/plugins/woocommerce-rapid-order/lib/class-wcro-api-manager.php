 ...
 */
if ( get_option( 'wcro_api_manager_activated' ) != 'Activated' ) {
    add_action( 'admin_notices', 'WCRO_API_Manager::am_example_inactive_notice' );
}

class WCRO_API_Manager {

	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API Manager server. If not set then the Author URI is used.
	public $upgrade_url = 'https://www.nickv.codes/';

	/**
	 * @var string
	 */
	public $version = WCRO_VERSION;

	/**
	 * @var string
	 * This version is saved after an upgrade to compare this db version to $version
	 */
	public $wcro_api_manager_version_name = 'plugin_wcro_api_manager_version';
	 ...
	 */
	public $text_domain = WC_Rapid_Order::TEXT_DOMAIN;
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'Rapid Order';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'wcro_api_manager';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'wcro_api_manager_product_id';
			$this->ame_instance_key 			= 'wcro_api_manager_instance';
			$this->ame_deactivate_checkbox_key 	= 'wcro_api_manager_deactivate_checkbox';
			$this->ame_activated_key 			= 'wcro_api_manager_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'wcro_api_deactivate_checkbox';
			$this->ame_activation_tab_key 			= 'wcro_api_manager_dashboard';
			$this->ame_deactivation_tab_key 		= 'wcro_api_manager_deactivation';
			$this->ame_settings_menu_title 			= 'Rapid Order';
			$this->ame_settings_title 				= 'Rapid Order';
			...
			$this->ame_plugin_name 			= 'woocommerce-rapid-order/woocommerce-rapid-order.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			$this->ame_product_id 			= get_option( $this->ame_product_id_key ); // Software Title
			$this->ame_renew_license_url 	= 'https://www.nickv.codes/my-account'; // URL to renew a license. Trailing slash in the upgrade_url is required.
	 ...
	 * API Key Class.
	 *
	 * @return WCRO_Api_Manager_Key
	 */
	public function key() {
		return WCRO_Api_Manager_Key::instance();
	}

	/**
	 * Update Check Class.
	 *
	 * @return WCRO_API_Manager_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return WCRO_API_Manager_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		$wcro_api_manager_password_management = new WCRO_API_Manager_Password_Management();

		// Generate a unique installation $instance id
		$instance = $wcro_api_manager_password_management->generate_password( 12, false );
		...
		$curr_ver = get_option( $this->wcro_api_manager_version_name );

		// checks if the current plugin version is lower than the version being installed
		if ( version_compare( $this->version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->wcro_api_manager_version_name, $this->version );
		}
	...
	public static function am_example_inactive_notice() { ?>
		...
		<?php if ( isset( $_GET['page'] ) && 'wcro_api_manager_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The Rapid Order API License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', WC_Rapid_Order::TEXT_DOMAIN ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wcro_api_manager_dashboard' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
...
function WCRO_API_Manager() {
    return WCRO_API_Manager::instance();
}
