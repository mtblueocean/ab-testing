...
if ( is_plugin_active( 'woocommerce-wholesale-prices/woocommerce-wholesale-prices.bootstrap.php' ) ) {
    ...
    if ( version_compare( $wwp_plugin_data[ 'Version' ] , '1.5.0' , '<' ) ) {
        ...
        /*
        |---------------------------------------------------------------------------------------------------------------
        | Apply wholesale prices on shop and calculations for custom product types
        |---------------------------------------------------------------------------------------------------------------
        */

        // Apply wholesale price to archive and single product pages ( Custom Product Types )
        add_filter( 'woocommerce_get_price_html' , array( $wc_wholesale_prices_premium , 'wholesalePriceHTMLFilter' ) , 10 , 2 );

        // Apply wholesale price upon adding product to cart ( Custom Product Types )
        add_filter( 'wwp_filter_get_custom_product_type_wholesale_price' , array( $wc_wholesale_prices_premium , 'applyCustomProductTypeWholesalePrice' ) , 10 , 3 );
        ...
        */
        ...
        // Display product category level per order quantity wholesale discount table ( On shop pages )
        add_filter( 'wwp_filter_wholesale_price_html' , array( $wc_wholesale_prices_premium->wwpp_wholesale_price_product_category , 'render_order_quantity_based_wholesale_discount_per_category_level_table_markup' ) , 120 , 4 );

        // Apply product category level per order quantity wholesale discount ( On cart/checkout page )
        add_filter( 'wwp_filter_wholesale_price_cart' , array( $wc_wholesale_prices_premium->wwpp_wholesale_price_product_category , 'apply_order_quantity_based_wholesale_discount_per_category_level' ) , 10 , 4 );
        ...
        add_filter( 'wwp_product_original_price' , array( $wc_wholesale_prices_premium , 'filter_product_original_price_visibility' ) , 10 , 5 );
        ...
        // Remove coupons for wholesale users when necessary
        add_action( 'woocommerce_before_cart' , array( $wc_wholesale_prices_premium , 'removeCouponsForWholesaleUsersWhenNecessary' ) );
        add_action( 'woocommerce_before_checkout_form' , array( $wc_wholesale_prices_premium , 'removeCouponsForWholesaleUsersWhenNecessary' ) );

        // Apply filters to override the default wholesale price suffix if the Override Price Suffix in the settings is set
        add_filter( 'wwp_filter_wholesale_price_display_suffix' , array( $wc_wholesale_prices_premium , 'overrideWholesalePriceSuffix' ) , 10 , 1 );
        add_filter( 'woocommerce_get_price_suffix' , array( $wc_wholesale_prices_premium , 'overrideRegularPriceSuffixForWholesaleRoles' ) , 10 , 2 );
