	...
	inlineEditPost.edit = function( id ) {
		...
		if ( $post_id > 0 ) {
            ...
            var $wc_inline_data = $( '#woocommerce_inline_' + $post_id ),
                ...
                $allowed_product_types = $( '.wholesale_minimum_order_quantity_allowed_product_types' , $post_row ).data( 'product_types' );

            if ( jQuery.inArray( $product_type , $allowed_product_types ) >= 0 ) {
                $( '.quick_edit_wholesale_minimum_order_quantity' , $edit_row ).show();
            } else {
                $( '.quick_edit_wholesale_minimum_order_quantity' , $edit_row ).hide();
            }
		}
	};
} );
