    ...
    class WWPP_WC_Product_Bundles {
        ...
        */
        ...
        public function filter_bundle_product_base_price( $bundle_price_data , $wc_product_bundle ) {
            ...
            $wholesale_price                   = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop( WWP_Helper_Functions::wwp_get_product_id( $wc_product_bundle ) , $user_wholesale_role );
        ...
        public function filter_bundle_item_price( $bundled_item_price , $product , $discount , $bundled_item ) {
            ...
            $wholesale_price = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop( $product_id , $user_wholesale_role ); // Per product wholesale pricing
        ...
        public function filter_variable_bundle_variations_data( $bundled_product_id , $bundled_item ) {
            ...
            if ( !empty( $user_wholesale_role ) ) {
                ...
                foreach ( $variations as $variation ) {
                    ...
                    $wholesale_price = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop( $variation[ 'variation_id' ] , $user_wholesale_role );
        ...
        public function filter_per_variation_price_html( $bundled_product_id , $bundled_item ) {
            ...
            if ( !empty( $user_wholesale_role ) ) {
                ...
                foreach ( $variations as $variation ) {
                    ...
                    $wholesalePrice   = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop( $variation[ 'variation_id' ] , $user_wholesale_role );
                    ...
                    if ( is_numeric( $wholesalePrice ) && !$has_per_order_quantity_wholesale_price_mapping ) {
                    ...
                    // Have category level per order quantity based discount mapping
                    if ( is_numeric( $wholesalePrice ) && !$has_per_cat_level_order_quantity_wholesale_discount_mapping ) {

                        $base_term_id = $this->_wwpp_wholesale_price_product_category->get_base_term_id( WWP_Helper_Functions::wwp_get_product_id( $product ) , $user_wholesale_role );

                        if ( $base_term_id ) {

                            $wholesale_cat_level_price = $this->_wwpp_wholesale_price_product_category->get_cat_level_wholesale_discount( $base_term_id , $variation[ 'variation_id' ] , $user_wholesale_role );

                            $mapping = get_term_meta( $base_term_id , 'wwpp_quantity_based_wholesale_discount_mapping' , true );
                            if ( !is_array( $mapping ) )
                                $mapping = array();

                            $has_mapping_entry = false;
                            foreach ( $mapping as $map )
                                if ( $map[ 'wholesale-role' ] === $user_wholesale_role[ 0 ] )
                                    $has_mapping_entry = true;

                            if ( $wholesale_cat_level_price && $has_mapping_entry )
                    ...
                    if ( is_numeric( $wholesalePrice ) ) {
                        ...
                        $wholesalePriceHTML = apply_filters( 'wwp_product_original_price' , '<del class="original-computed-price">' . WWP_Helper_Functions::wwp_formatted_price( $currVarPrice ) . $product->get_price_suffix() . '</del>' , $wholesalePrice , $currVarPrice , $product , $user_wholesale_role );
                        ...
                        $wholesalePriceHTML = apply_filters( 'wwp_filter_wholesale_price_html' , $wholesalePriceHTML , $currVarPrice , $variationProduct , $user_wholesale_role , $wholesalePriceTitleText , $wholesalePrice );
