        ...
        */
        ...
        public function load_back_end_styles_and_scripts( $handle ) {
            ...
            if ( ( $handle == 'post-new.php' || $handle == 'post.php' ) && $post_type == 'product' ) {
                ...
                wp_enqueue_script( 'wwpp_chosen_js' , WWPP_JS_URL . 'lib/chosen/chosen.jquery.min.js' , array('jquery') , $this->_wwpp_current_version , true );
                wp_enqueue_script( 'wwpp_backEndAjaxServices_js' , WWPP_JS_URL . 'app/modules/BackEndAjaxServices.js' , array( 'jquery' ) , $this->_wwpp_current_version , true );
                wp_enqueue_script( 'wwpp_single_product_admin_js' , WWPP_JS_URL . 'app/wwpp-single-product-admin.js' , array( 'jquery' ) , $this->_wwpp_current_version , true );
                ...
                wp_localize_script( 'wwpp_single_variable_product_admin_custom_bulk_actions_js' , 'wwpp_custom_bulk_actions_params' , array(
                    ...
                    'i18n_prompt_message' => __( 'Enter a value (leave blank to remove min order quantity)' , 'woocommerce-wholesale-prices-premium' )
                ) );
                    ...
                    if ( WWP_Helper_Functions::wwp_get_product_type( $product ) == 'variable' ) {
                        ...
                        if ( !empty( $userWholesaleRole ) ) {
                            ...
                            foreach ( $variations as $variation ) {
                                ...
                                $wholesalePrice = WWP_Wholesale_Prices::get_product_wholesale_price_on_shop( $variation[ 'variation_id' ] , $userWholesaleRole );
                                ...
                                if ( !empty( $wholesalePrice ) && !$has_per_cat_level_order_quantity_wholesale_discount_mapping ) {
                                    ...
                                    $base_term_id = $this->_wwpp_wholesale_price_product_category->get_base_term_id( WWP_Helper_Functions::wwp_get_product_id( $product ) , $userWholesaleRole );

                                    if ( $base_term_id ) {

                                        $wholesale_cat_level_price = $this->_wwpp_wholesale_price_product_category->get_cat_level_wholesale_discount( $base_term_id , $variation[ 'variation_id' ] , $userWholesaleRole );
                                        ...
                                        foreach ( $mapping as $map ) {
                                            ...
                                            if ( $map[ 'wholesale-role' ] === $userWholesaleRole[ 0 ] ) {
                            ...
                            if ( !WWP_Helper_Functions::is_plugin_active( 'woocommerce-min-max-quantities/woocommerce-min-max-quantities.php' ) ) // Only load this if min max quantities plugin is not active, if its active we already integrated with it
